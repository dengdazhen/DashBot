...
def _encoding_to_transform(
    ...
    groupby: List[str] = []
    # groupby: List[str] = [
    #     enc["field"] for enc in new_encoding.values() if "field" in enc and enc["field"] not in groupby
    # ]
    for enc in new_encoding.values():
        if "field" in enc and enc["field"] not in groupby:
            groupby.append(enc["field"])
