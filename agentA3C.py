...
class A3C(agent.AttributeSavingMixin, agent.AsyncAgent):
	...
	def update(self, statevar, info):
		...
		if self.recurrent:
			...
			batch_vec, batch_v = self.model(batch_obs)	
		self.constraint.set_df(info["dataset_idx"])
		batch_distrib, _ = self.constraint.chained_sampling_batch(batch_obs, batch_vec)
		rev_returns = []
		...
		pi_loss = 0
		for a_id in range(len(self.past_action[self.t_start])):
			# the same action across time stamps
			batch_action = torch.stack(
				[self.past_action[i][a_id] for i in range(self.t_start, self.t)]
			)
			batch_log_prob = batch_distrib[a_id].log_prob(batch_action)
			# entropy of the same actions across time stamps
			batch_entropy = batch_distrib[a_id].entropy()
	...
	def act(self, obs, info):
		if self.training:
			return self._act_train(obs, info)
		else:
			return self._act_eval(obs, info)

	def observe(self, obs, reward, done, reset, info):
		if self.training:
			self._observe_train(obs, reward, done, reset, info)
		else:
			self._observe_eval(obs, reward, done, reset, info)

	def _act_train(self, obs, info):
		...
		with torch.no_grad():
			...
			# Size statevar: [1, 10, mv_feat_len], where N is the number of choices
			if self.recurrent:
				...
				avec, vout = self.model(statevar)
			# Size pouts[0]: [1, N], where N is the number of choices
			# Do not backprop through sampled actions
			self.constraint.set_df(info["dataset_idx"])
			pouts, actions = self.constraint.chained_sampling_batch(statevar, avec)

			self.past_action[self.t] = [action[0].detach() for action in actions]
			actions = [action.cpu().numpy()[0] for action in actions]
	...
	def _observe_train(self, obs, reward, done, reset, info):
		self.t += 1
		self.past_rewards[self.t - 1] = reward
		if self.process_idx == 0:
			logger.debug(
				"t:%s action:%s reward:%s", self.t, '-'.join([str(action.item()) for action in self.past_action[self.t - 1]]), reward
			)
		if self.t - self.t_start == self.t_max or done or reset:
			if done:
				...
				statevar = self.batch_states([obs], self.device, self.phi) 
			self.update(statevar, info)
		if done or reset:
			self.train_recurrent_states = None

	def _act_eval(self, obs, info):
		...
		with torch.no_grad(), pfrl.utils.evaluating(self.model):
			...
			if self.recurrent:
				...
				batch_vec, _ = self.model(statevar)
				self.constraint.set_df(info["dataset_idx"])
				_, actions = self.constraint.chained_sampling_batch(statevar, batch_vec)
	...
	def _observe_eval(self, obs, reward, done, reset, info):
		if done or reset:
			self.test_recurrent_states = None
